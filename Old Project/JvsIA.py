...
class JoueurVSIA():
    ...
    # Fermer les fenêtres quand la partie est finie:
    ...
    def bestPokemonIA(self,pokemonAdverses):
        ...
        if isinstance(type2adverse,str):
            ...
            possiblePokemons=self.deck2[(self.deck2["Type 1"].isin(bestTypes)) & (self.deck2["Total"]>=pokemonAdverses["Total"])]
        if len(possiblePokemons)>0:
            idx = random.choice(possiblePokemons.index.tolist())
            return possiblePokemons.loc[idx]
        else:
            possiblePokemons=self.deck2[(self.deck2["Type 1"].isin(bestTypes) | self.deck2["Type 2"].isin(bestTypes))].sort_values(by="Total",ascending=False)
            if len(possiblePokemons)==0:
