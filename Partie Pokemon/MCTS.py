...
class Plateau:
    ...
    def jouer(self, gl, gc, l, c):
        ...
        #si il y en a un, on considère qu'on gagne forcément le combat avec l'adversaire
        else:
...
class Noeud:
    ...
    def scoreUCB1(self, c):
        exploitation = self.score / self.visites
        exploration = c * sqrt(log(self.parent.visites) / self.visites)
...
def MCTS(grille, gagnantsSousGrilles, pokemons_places, joueur, case, iterations=200, c=sqrt(2)):
