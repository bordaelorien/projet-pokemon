...
class IAVSIA:
    ...
    def pokemonChoisiOpti(self, joueur, pokemon_adverse):
        ...
        if isinstance(type2adverse, str):
            ...
            possiblePokemons = deck[
                ((deck["Type 1"].isin(bestTypes)) | (deck["Type 2"].isin(bestTypes)))
                & (deck["Total"] >= pokemon_adverse["Total"])]
        else:
            possiblePokemons = deck[
                (deck["Type 1"].isin(bestTypes)) &
                (deck["Total"] >= pokemon_adverse["Total"])]
