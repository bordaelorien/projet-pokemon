...
class JoueurVSIA():
    ...
    def __init__(self,nbrePokemons):
        # GRAPHIQUE :
        self.fenetre = tk.Tk()
        ...
        self.deck1, self.deck2 = db.decks(dl.dfPokemon(), nbrePokemons)
        self.matriceAdj = dl.dfMatriceAdjacence()
        self.matriceAdj.set_index('Type Attaquant', inplace=True)
    ...
    def bestPokemonIA(self, pokemonAdverses):
        ...
        bestTypes = self.matriceAdj[self.matriceAdj[type1adverse] > 1].index.tolist()

        if isinstance(type2adverse, str):
            bestTypes2 = self.matriceAdj[self.matriceAdj[type2adverse] > 1].index.tolist()
            bestTypes = bestTypes + bestTypes2

            possiblePokemons = self.deck2[
                ((self.deck2["Type 1"].isin(bestTypes)) | (self.deck2["Type 2"].isin(bestTypes)))
                & (self.deck2["Total"] >= pokemonAdverses["Total"])]
        else:
            possiblePokemons = self.deck2[
                (self.deck2["Type 1"].isin(bestTypes)) &
                (self.deck2["Total"] >= pokemonAdverses["Total"])]
        
        if len(possiblePokemons) > 0:
            
            return possiblePokemons.sample(1).iloc[0]
        ...
        if len(possiblePokemons) > 0:
            ...
            return possiblePokemons.iloc[0]
        ...
        return self.deck2.sample(1).iloc[0]
